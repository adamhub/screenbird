...
public class Encoder {
    ...
    public void merge(File source1,File source2,File target) throws EncoderException {
        ...
        this.convertMp4ToMpg(source1, tempMpg1);
        this.convertMp4ToMpg(source2, tempMpg2);
        FFMPEGExecutor ffmpeg = locator.createExecutor();
        try {
    ...
    private void convertMpgToMp4(File tempTarget, File target) throws EncoderException{
        target = target.getAbsoluteFile();
        target.getParentFile().mkdirs();
        FFMPEGExecutor ffmpeg = locator.createExecutor();
        ffmpeg.addArgument("-i");
