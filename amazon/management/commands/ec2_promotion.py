...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        ssh_base = ["""/usr/bin/ssh""", """-i""", pem_path, """-o""", 
                    """StrictHostKeyChecking no""", "ubuntu@%s" % node]

        # Transfer ffmpeg.py
        cmd = ["scp", "-i", pem_path, os.path.join(settings.PROJECT_ROOT,
               "amazon", "ec2_files", "ffmpeg.py"), "ubuntu@%s:~" % node]
        ...
        # Transfer encoder.py
        cmd = ["scp", "-i", pem_path, os.path.join(settings.PROJECT_ROOT,
               "amazon", "ec2_files", "encoder.py"), "ubuntu@%s:~" % node]
        p = subprocess.Popen(cmd, stderr=subprocess.PIPE, stdout=subprocess.PIPE)
        msg = p.communicate()
        if msg[1] != '':
        ...
        cmd = ["scp", "-i", pem_path, os.path.join(settings.PROJECT_ROOT,
               "videos", "dictconfig.py"), "ubuntu@%s:~" % node]
        p = subprocess.Popen(cmd, stderr=subprocess.PIPE, stdout=subprocess.PIPE)
        msg = p.communicate()
        if msg[1] != '':
        ...
        cmd = ["scp", "-i", pem_path, os.path.join(settings.PROJECT_ROOT,
               "videos", "cocreate.py"), "ubuntu@%s:~" % node]
        p = subprocess.Popen(cmd, stderr=subprocess.PIPE, stdout=subprocess.PIPE)
        msg = p.communicate()
        if msg[1] != '':
